...
description: Stop and hand this Claude Code session off to another machine. Writes a HANDOFF.md
  state note, optionally commits work-in-progress, and (only if the project has a .codesync
  marker) runs the Syncthing push so the code and this transcript are ready for the other
  machine to resume. Use right before switching computers. Invoked as /codesync:stop.
disable-model-invocation: true
---

# /codesync:stop — hand this session off
...
3. **Push the sync (gated).** Check for a `.codesync` file at the project root:
   - If it exists, run the `codesync stop` command and report the status it prints (it rescans the
...
4. **Tell the user how to resume.** On the other machine, run `codesync start` (in the terminal) —
...
transcript, the user can optionally run the `codesync stop` command once more after exiting Claude.
